...
dst = np.array([[offset,720], 
                [offset, 0], 
                [1280 - offset, 0], 
                [1280 - offset, 720]], dtype=np.float32)
...
def apply_sliding_windows(binary_warped_l,binary_warped_r, nwindows=9, margin=100, minpix=55):
    ...
    midpoint = np.int64(binary_warped_l.shape[0]//2)
    ...
    window_height = np.int64(binary_warped_l.shape[0]//nwindows)
    ...
    for window in range(nwindows):
        ...
        if len(good_left_inds) > minpix:
            leftx_current = np.int64(np.mean(nonzerox_l[good_left_inds]))
       
            
        if len(good_right_inds) > minpix:        
            rightx_current = np.int64(np.mean(nonzerox_r[good_right_inds]))
...
def project_measurements(image, binary_warped_l,binary_warped_r, Minv):
    ...
    cv2.putText(out_image, 'Radius of Curvature = {:4d}(m)'.format(curvature), (40, 650), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.6, (255, 255, 255), 2, cv2.LINE_AA)
    ...
    cv2.putText(out_image, 'Vehicle is {:.2f}(m) {} of center'.format(abs(position), side), (40, 690), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.6, (255, 255, 255), 2, cv2.LINE_AA)
...
def pipline_outputs_combined_cars(main_frame,stages_output):
    #Get the orginal frame height
    h1, w1 = main_frame.shape[:2]
    #Get pipeline output images size
    square_size=int((w1-((1/4)*w1))/len(stages_output))        
...
def detect_image(image_cv2):
    image_cars=image_cv2.copy()
    if debug:
        image_cv2=process_image_debug(image_cv2)
    else:
        image_cv2=process_image(image_cv2)
    results = model(image_cars)
    ...
    if debug and not(len(cropped_car_images)==0):
        return pipline_outputs_combined_cars(image_cv1,cropped_car_images)
    return image_cv1
